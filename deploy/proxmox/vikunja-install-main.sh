...
show_deployment_summary() {
    ...
    if [[ "$mcp_http_enabled" == "true" ]]; then
        ...
        echo ""
        echo "Health Check:      http://${IP_ADDRESS%/*}:${mcp_http_port}/health"
        echo "MCP Endpoint:      http://${IP_ADDRESS%/*}:${mcp_http_port}/mcp"
        echo "SSE Endpoint:      http://${IP_ADDRESS%/*}:${mcp_http_port}/sse"
        echo ""
        echo "Authentication:    Vikunja API token required"
        echo "Transport Type:    HTTP Streamable (primary), SSE (deprecated)"
        echo ""
        echo "Example connections:"
        echo "  Health check:    curl http://${IP_ADDRESS%/*}:${mcp_http_port}/health"
        echo ""
        echo "  n8n:             POST http://${IP_ADDRESS%/*}:${mcp_http_port}/mcp"
        echo "                   Header: Authorization: Bearer YOUR_TOKEN"
        echo "                   Body: JSON-RPC 2.0 request"
        echo ""
        echo "  SSE (legacy):    curl -N -H \"Authorization: Bearer TOKEN\" \\"
        echo "                   http://${IP_ADDRESS%/*}:${mcp_http_port}/sse"
        echo ""
    else
        echo "Status:            ⚠️  DISABLED (stdio mode)"
        echo ""
        echo "HTTP transport is disabled. MCP server runs in stdio mode only."
        echo "To enable HTTP transport, set MCP_HTTP_ENABLED=true in deployment config."
        echo ""
    fi
    
    echo "See deploy/proxmox/README.md for configuration details"
    echo ""
