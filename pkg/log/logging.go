...
func NewHTTPLogger(configLogEnabled bool, configLogHTTP string, configLogFormat string) *slog.Logger {
	var writer = io.Discard
	if configLogEnabled && configLogHTTP != "off" {
		writer = GetLogWriter(configLogHTTP, "http")
	}
...
func ConfigureLogger(configLogEnabled bool, configLogStandard string, configLogPath string, configLogLevel string, configLogFormat string) {
	...
	format := strings.ToLower(configLogFormat)
	...
	if configLogEnabled && configLogStandard != "off" {
		writer = GetLogWriter(configLogStandard, "standard")
	}

	handler := CreateHandler(writer, level, format)
	logInstance = slog.New(handler)
}
