...
// UserExportService handles all business logic for user data export functionality
type UserExportService struct {
	DB *xorm.Engine
}

// NewUserExportService creates a new instance of UserExportService
func NewUserExportService(engine *xorm.Engine) *UserExportService {
	return &UserExportService{
		DB: engine,
	}
}

// ExportUserData creates a complete data export for a user
func (ues *UserExportService) ExportUserData(s *xorm.Session, u *user.User) error {
	...
	// Send a notification
	return notifications.Notify(u, &models.DataExportReadyNotification{
		User: u,
	})
}

func (ues *UserExportService) exportProjectsAndTasks(s *xorm.Session, u *user.User, wr *zip.Writer) (taskIDs []int64, err error) {
	// Get all projects
	rawProjects, _, _, err := models.GetRawProjectsForUser(
		s,
		&models.ProjectOptions{
			Search:      "",
			User:        u,
			Page:        0,
			PerPage:     -1,
	...
	projectsMap := make(map[int64]*models.ProjectWithTasksAndBuckets, len(rawProjects))
	projectIDs := []int64{}
	for _, p := range rawProjects {
		...
		pp := &models.ProjectWithTasksAndBuckets{
			Project: *p,
		}
	...
	views := map[int64]*models.ProjectView{}
	...
	for _, v := range views {
		if projectsMap[v.ProjectID].Views == nil {
			projectsMap[v.ProjectID].Views = []*models.ProjectView{}
		}
		projectsMap[v.ProjectID].Views = append(projectsMap[v.ProjectID].Views, v)
		viewIDs = append(viewIDs, v.ID)
	}

	tasks, _, _, err := models.GetTasksForProjects(s, rawProjects, u, models.NewTaskSearchOptions(
		"",    // search
		0,     // page
		-1,    // perPage
		nil,   // sortby
	...
	taskMap := make(map[int64]*models.TaskWithComments, len(tasks))
	for _, t := range tasks {
		taskMap[t.ID] = &models.TaskWithComments{
			Task: *t,
		}
	...
	for _, tb := range taskBuckets {
		...
		if projectsMap[view.ProjectID].TaskBuckets == nil {
			projectsMap[view.ProjectID].TaskBuckets = []*models.TaskBucket{}
		}
	...
	for _, p := range taskPositions {
		...
		if projectsMap[view.ProjectID].Positions == nil {
			projectsMap[view.ProjectID].Positions = []*models.TaskPosition{}
		}
...
func (ues *UserExportService) exportTaskAttachments(s *xorm.Session, wr *zip.Writer, taskIDs []int64) error {
	tas, err := models.GetTaskAttachmentsByTaskIDs(s, taskIDs)
	if err != nil {
		return err
	}
...
func (ues *UserExportService) exportSavedFilters(s *xorm.Session, u *user.User, wr *zip.Writer) error {
	filters, err := models.GetSavedFiltersForUser(s, u, "")
	if err != nil {
		return err
	}
...
func (ues *UserExportService) exportProjectBackgrounds(s *xorm.Session, u *user.User, wr *zip.Writer) error {
	projects, _, _, err := models.GetRawProjectsForUser(
		s,
		&models.ProjectOptions{
			User: u,
			Page: -1,
...
// RegisterOldExportCleanupCron registers a cron job to clean up old export files
