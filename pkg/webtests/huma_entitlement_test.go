...
func TestHumaEntitlement_Gates(t *testing.T) {
	t.Run("time entries 403 for a user whose plan lacks the feature", func(t *testing.T) {
		...
		rec = humaRequest(t, e, http.MethodPost, "/api/v2/time-entries/timer/stop", "", humaTokenFor(t, &testuser16), "")
		assert.Equal(t, http.StatusForbidden, rec.Code, rec.Body.String())
		assert.Contains(t, rec.Body.String(), `"code":20002`)
	})

	t.Run("every time entry endpoint is gated", func(t *testing.T) {
		e, err := setupTestEnv()
		require.NoError(t, err)
		license.SetForTests([]license.Feature{license.FeatureTimeTracking})
		defer license.ResetForTests()
		...
		for _, tc := range []struct {
			...
			{http.MethodGet, "/api/v2/time-entries/1", ""},
			{http.MethodPut, "/api/v2/time-entries/1", `{"project_id":37,"start_time":"2024-01-01T10:00:00Z"}`},
			{http.MethodDelete, "/api/v2/time-entries/1", ""},
			{http.MethodGet, "/api/v2/tasks/1/time-entries", ""},
			{http.MethodGet, "/api/v2/projects/1/time-entries", ""},
		} {
			t.Run(tc.method+" "+tc.path, func(t *testing.T) {
				rec := humaRequest(t, e, tc.method, tc.path, tc.body, humaTokenFor(t, &testuser16), "")
				assert.Equal(t, http.StatusForbidden, rec.Code, rec.Body.String())
				assert.Contains(t, rec.Body.String(), `"code":20002`)
			})
		}
