...
// Copyright 218-present Vikunja and contributors. All rights reserved.
//
...
func TestTaskV2All(t *testing.T) {
	th := NewTestHelper(t)
	th.Login(t, &testuser1)

	t.Run("Normal", func(t *testing.T) {
		rec, err := th.Request(t, "GET", "/api/v2/tasks", nil)
	...
	t.Run("Pagination", func(t *testing.T) {
		rec, err := th.Request(t, "GET", "/api/v2/tasks?page=1&per_page=3", nil)
		require.NoError(t, err)
		assert.Equal(t, http.StatusOK, rec.Code)
	...
	t.Run("Filtering", func(t *testing.T) {
		rec, err := th.Request(t, "GET", "/api/v2/tasks?s="+url.QueryEscape("task #1"), nil)
		require.NoError(t, err)
		assert.Equal(t, http.StatusOK, rec.Code)
		assert.Contains(t, rec.Body.String(), `"title":"task #1"`)
