...
func TestMCP_Tools_TaskLifecycle(t *testing.T) {
	...
	toolResultJSON(t, c.callTool("tasks_create", map[string]any{"project": 1, "title": "mcp task", "description": "**keep me**", "priority": 4, "hex_color": "ff8800"}), &task)
	id := int64(task["id"].(float64))
	assert.Equal(t, "**keep me**", task["description"])
	require.NotContains(t, c.callTool("tasks_update", map[string]any{"projecttask": id, "done": true}), "isError")
	...
	require.NotContains(t, c.callTool("tasks_update", map[string]any{"projecttask": id, "done": false}), "isError")
	toolResultJSON(t, c.callTool("tasks_read", map[string]any{"projecttask": id}), &task)
...
func TestMCP_Tools_ListEnvelopeAndFilter(t *testing.T) {
	...
	toolResultJSON(t, c.callTool("tasks_list", map[string]any{"filter": "done = false", "per_page": 2, "page": 1}), &env)
	for _, k := range []string{"items", "total", "page", "per_page", "total_pages"} {
...
func TestMCP_Tools_CommentLifecycle(t *testing.T) {
	...
	toolResultJSON(t, c.callTool("task_comments_create", map[string]any{"task": 1, "comment": "from mcp"}), &comment)
	...
	res := c.callTool("task_comments_update", map[string]any{"task": 1, "commentid": id, "comment": "edited"})
	require.NotContains(t, res, "isError", toolResultText(t, res))
	toolResultJSON(t, c.callTool("task_comments_read", map[string]any{"task": 1, "commentid": id}), &comment)
	assert.Equal(t, "edited", comment["comment"])
}
func TestMCP_Tools_AssigneeAddRemove(t *testing.T) {
	c := newMCPClient(t, mcpFullToken)
	require.NotContains(t, c.callTool("task_assignees_create", map[string]any{"projecttask": 1, "user_id": 1}), "isError")
	require.NotContains(t, c.callTool("task_assignees_delete", map[string]any{"projecttask": 1, "user": 1}), "isError")
}
...
func TestMCP_Tools_ScopeDeniedIsError(t *testing.T) {
	...
	res := c.callTool("do_action", map[string]any{"action": "projects_create", "arguments": map[string]any{"title": "nope"}})
	assert.Equal(t, true, res["isError"])
	assert.Contains(t, toolResultText(t, res), "not authorized")
}
func TestMCP_Tools_ValidationIsError(t *testing.T) {
	c := newMCPClient(t, mcpFullToken)
	res := c.callTool("tasks_create", map[string]any{"project": 1, "title": ""})
	assert.Equal(t, true, res["isError"])
...
func TestMCP_Tools_UnknownArgumentIsError(t *testing.T) {
	c := newMCPClient(t, mcpFullToken)
	res := c.callTool("tasks_read", map[string]any{"projecttask": 1, "bogus": true})
	assert.Equal(t, true, res["isError"])
...
func TestMCP_Tools_CreateSchemaMarksRequired(t *testing.T) {
	...
	for _, raw := range resp["result"].(map[string]any)["tools"].([]any) {
		...
		assert.ElementsMatch(t, []any{"project", "title"}, schema["required"])
...
func TestMCP_Tools_RESTValidationIsError(t *testing.T) {
	c := newMCPClient(t, mcpFullToken)
	res := c.callTool("tasks_create", map[string]any{"project": 1, "title": "invalid repeat", "repeat_after": -1})
	assert.Equal(t, true, res["isError"])
...
func TestMCP_Tools_ExpandRequiresScopeOnLoopback(t *testing.T) {
	c := newMCPClient(t, mcpFullToken)
	allowed := c.callTool("tasks_read", map[string]any{"projecttask": 1, "expand": []string{"comments"}})
	require.NotContains(t, allowed, "isError", toolResultText(t, allowed))
	denied := c.callTool("tasks_read", map[string]any{"projecttask": 1, "expand": []string{"reactions"}})
...
func TestMCP_Tools_ProjectAndLabelLifecycle(t *testing.T) {
	for _, resource := range []string{"projects", "labels"} {
		t.Run(resource, func(t *testing.T) {
			c := newMCPClient(t, mcpFullToken)
			var item map[string]any
			toolResultJSON(t, c.callTool(resource+"_create", map[string]any{"title": "mcp item", "hex_color": "ff8800"}), &item)
			id := item["id"]
			updated := c.callTool(resource+"_update", map[string]any{"id": id, "title": "edited"})
