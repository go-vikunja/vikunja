...
func IssueUserToken(u *user.User, deviceInfo, ipAddress string, long bool) (*IssuedUserToken, error) {
	s := db.NewSession()
	defer s.Close()
...
func NewUserAuthTokenResponse(u *user.User, c *echo.Context, long bool) error {
	token, err := IssueUserToken(u, c.Request().UserAgent(), c.RealIP(), long)
	if err != nil {
		return err
	}
