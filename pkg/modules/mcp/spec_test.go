...
func describeFor(t *testing.T, api huma.API, method, path string) string {
	t.Helper()
	item := api.OpenAPI().Paths[path]
	require.NotNil(t, item)
	var op *huma.Operation
	...
	require.NotNil(t, op)
	return describe(api.OpenAPI(), op, specFor(t, api, method, path))
}

func TestDescribe_FormatHint(t *testing.T) {
	api := newTestAPI(t)
	assert.Contains(t, describeFor(t, api, http.MethodGet, "/things"), formatArgHint)
	assert.NotContains(t, describeFor(t, api, http.MethodGet, "/things/{id}"), formatArgHint)
	assert.Contains(t, describeFor(t, newRichTextAPI(t), http.MethodPatch, "/notes/{id}"), formatArgHint)
}
