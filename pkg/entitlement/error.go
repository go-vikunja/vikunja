...
// ErrFeatureNotLicensed is a bare 404 (no error code) so unlicensed routes stay indistinguishable from unregistered ones.
...
// ErrInvalidValue: the value does not fit the feature - flags take 0/1, limits
// take a non-negative maximum.
...
func (err ErrInvalidValue) Error() string {
	return fmt.Sprintf("Invalid value %d for entitlement feature %s", err.Value, err.Feature)
}

const ErrCodeInvalidValue = 20003

func (err ErrInvalidValue) HTTPError() web.HTTPError {
	return web.HTTPError{
		HTTPCode: http.StatusBadRequest,
		Code:     ErrCodeInvalidValue,
		Message:  fmt.Sprintf("The value %d is not valid for the feature %s.", err.Value, err.Feature),
	}
}
