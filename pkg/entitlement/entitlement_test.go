...
// The user_entitlements fixture rows for user 16, unchanged.
var fixtureRows16 = map[Feature]int64{
	FeatureTimeTracking:    0,
	FeatureTeamCreation:    0,
	FeatureMaxProjects:     1,
	FeatureMaxStorageBytes: 1024,
...
func TestReplace(t *testing.T) {
	...
	t.Run("rejects unknown and instance-only features", func(t *testing.T) {
		...
		rows, err := Rows(s, 16)
		require.NoError(t, err)
		assert.Equal(t, fixtureRows16, rows)
	})
	t.Run("rejects invalid values", func(t *testing.T) {
		...
		assert.True(t, IsErrInvalidValue(Replace(s, 16, map[Feature]int64{FeatureMaxProjects: -1})))
		assert.True(t, IsErrInvalidValue(Replace(s, 16, map[Feature]int64{FeatureTimeTracking: 7})))
