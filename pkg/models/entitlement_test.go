...
func TestEntitlementUsage(t *testing.T) {
	...
	got, err := EntitlementUsage(s, 1, []entitlement.Feature{
		entitlement.FeatureMaxProjects,
		entitlement.FeatureMaxStorageBytes,
	})
	require.NoError(t, err)
	...
	got, err = EntitlementUsage(s, 6, []entitlement.Feature{entitlement.FeatureMaxStorageBytes})
	require.NoError(t, err)
	assert.Equal(t, map[entitlement.Feature]int64{entitlement.FeatureMaxStorageBytes: 100}, got)

	got, err = EntitlementUsage(s, 1, nil)
	require.NoError(t, err)
	assert.Empty(t, got)
}
