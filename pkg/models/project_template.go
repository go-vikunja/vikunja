...
func (pt *ProjectTemplate) Create(s *xorm.Session, doer web.Auth) (err error) {
	...
	// Use ProjectDuplicate to copy the project. DuplicateShares stays false so
	// permissions and link shares are not carried over into the template.
	pd := &ProjectDuplicate{
	...
	pd.Project.ParentProjectID = noParentProjectID()
