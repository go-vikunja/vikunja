...
func TestRememberServesMemo(t *testing.T) {
	db.LoadAndAssertFixtures(t)

	t.Run("memoizes until a write", func(t *testing.T) {
		s := db.NewSession()
		defer s.Close()
		...
		first, err := db.Remember(s, "remember-test", fetch)
		require.NoError(t, err)
		second, err := db.Remember(s, "remember-test", fetch)
		require.NoError(t, err)
		assert.Equal(t, 42, first)
		assert.Equal(t, 42, second)
		assert.Equal(t, 1, calls)

		_, err = s.ID(1).Cols("title").Update(&Task{Title: "x"})
		require.NoError(t, err)

		third, err := db.Remember(s, "remember-test", fetch)
		require.NoError(t, err)
		assert.Equal(t, 42, third)
		assert.Equal(t, 2, calls)
	})

	t.Run("fetches only missing ids", func(t *testing.T) {
		s := db.NewSession()
		defer s.Close()
		...
		first, err := db.RememberEach(s, []int64{1, 2}, key, fetch)
		require.NoError(t, err)
		assert.Equal(t, map[int64]string{1: "v1", 2: "v2"}, first)

		second, err := db.RememberEach(s, []int64{2, 3}, key, fetch)
		require.NoError(t, err)
		assert.Equal(t, map[int64]string{2: "v2"}, second)

		third, err := db.RememberEach(s, []int64{3}, key, fetch)
		require.NoError(t, err)
		assert.Empty(t, third)

		assert.Equal(t, [][]int64{{1, 2}, {3}, {3}}, fetched)
	})
}
