...
func TestProject_Delete(t *testing.T) {
	t.Run("normal", func(t *testing.T) {
		...
		// Soft-deleted tasks survive too so a restore brings them back
		db.AssertExists(t, "tasks", map[string]interface{}{
			"id": 51,
		}, false)
	})
	...
	t.Run("soft-deleted projects are excluded from ReadAll", func(t *testing.T) {
		...
		// Project 44 is soft-deleted and owned by user 1
		p := &Project{}
		...
		for _, proj := range projectList {
			assert.NotEqual(t, int64(44), proj.ID, "Soft-deleted project 44 should not appear in ReadAll")
			assert.NotEqual(t, int64(45), proj.ID, "Soft-deleted project 45 should not appear in ReadAll")
			assert.NotEqual(t, int64(46), proj.ID, "Soft-deleted project 46 should not appear in ReadAll")
		}
	})
...
func TestProject_Restore(t *testing.T) {
	t.Run("restore soft-deleted project", func(t *testing.T) {
		...
		// Project 44 is soft-deleted and owned by user 1
		project, err := RestoreProject(s, 44, &user.User{ID: 1})
		require.NoError(t, err)
	...
	t.Run("restore soft-deleted parent restores children", func(t *testing.T) {
		...
		// Project 45 is soft-deleted parent, 46 is soft-deleted child
		_, err := RestoreProject(s, 45, &user.User{ID: 1})
		require.NoError(t, err)
		err = s.Commit()
		require.NoError(t, err)

		assertNotSoftDeleted(t, 45)
		assertNotSoftDeleted(t, 46)
	})
	...
	t.Run("restore project without admin access", func(t *testing.T) {
		...
		// Project 44 is owned by user 1, user 2 has no access
		_, err := RestoreProject(s, 44, &user.User{ID: 2})
		require.Error(t, err)
		assert.True(t, IsErrGenericForbidden(err))
	})
	t.Run("restore project shared with admin permission", func(t *testing.T) {
		db.LoadAndAssertFixtures(t)
		s := db.NewSession()
		defer s.Close()

		// User 6 is not the owner of 44 but has an admin share on it
		_, err := RestoreProject(s, 44, &user.User{ID: 6})
		require.NoError(t, err)
		err = s.Commit()
		require.NoError(t, err)
...
func TestProject_GetDeletedProjects(t *testing.T) {
	t.Run("returns deleted projects for user", func(t *testing.T) {
		...
		// User 1 owns soft-deleted projects 44, 45, 46
		...
		assert.True(t, ids[44])
		assert.True(t, ids[45])
		assert.True(t, ids[46])
	})
...
func TestProject_PermanentDelete(t *testing.T) {
	t.Run("permanently deletes project and all related entities", func(t *testing.T) {
		...
		// Soft-deleted tasks go too, there is nothing left to restore them into
