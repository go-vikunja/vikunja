...
func checkProjectBeforeUpdateOrDelete(s *xorm.Session, project *Project) (err error) {
	...
	if project.IsTemplate && parentID != 0 {
...
// checkDefaultProjectRestrictions rejects states a user's default project cannot enter.
func checkDefaultProjectRestrictions(s *xorm.Session, project *Project) (err error) {
	if !project.IsArchived && !project.IsTemplate {
		return nil
	}

	isDefaultProject, err := project.isDefaultProject(s)
	if err != nil || !isDefaultProject {
		return err
	}
...
func UpdateProject(s *xorm.Session, project *Project, auth web.Auth, updateProjectBackground bool) (err error) {
	...
	err = checkDefaultProjectRestrictions(s, project)
	if err != nil {
		return err
	}
