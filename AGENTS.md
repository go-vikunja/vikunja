...
- Frontend code must use the generated API client and types in `frontend/src/client/generated` plus the query modules in `frontend/src/client/queries`. The legacy model/service layer (`frontend/src/models`, `frontend/src/modelTypes`, `frontend/src/services`, Axios) is deleted; `frontend/eslint.config.js` errors on importing it.
