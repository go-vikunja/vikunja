...
- **Service Layer Tests** (`pkg/services/*_test.go`): Unit tests for business logic, don't require fixtures
  - Run with: `go test ./pkg/services -v`
  - Test individual service methods and business logic
  - Use in-memory database or mocked dependencies
- **Model Layer Tests** (`pkg/models/*_test.go`): Test database operations and model methods
  - May require fixtures for complex relationship testing
- **Web/Integration Tests** (`pkg/webtests/`): Full HTTP API testing with fixtures
  - Must use: `mage test:web` (handles fixture loading and environment setup)
  - Test complete request/response cycles
  - Require all fixtures to be loaded for realistic testing scenarios
...
- Always test both positive and negative authorization scenarios
- Use test fixtures in `pkg/db/fixtures/` for consistent test data (web tests only)
- Write service layer tests for complex business logic (no fixtures needed)
- Use descriptive test names that explain the scenario being tested
- Consider test isolation - some tests may modify shared data
...
- **XORM Query Issues**: When using `Find()` with maps, use slices instead to avoid "multiple primary key" errors
- **Files Package Dependencies**: The `files` package requires global initialization (`files.SetEngine()`, `files.InitFileHandler()`) for full functionality
- **Test Environment**: Service layer tests don't need fixtures, but web tests require `mage test:web` for proper fixture loading
- **Go Version**: Ensure Go 1.21+ is installed. Older versions may cause compilation or runtime issues
...
4.  **Run Tests**:
    ...
    *   Run `mage test:web` to run the web tests (requires fixtures).
    *   Run `go test ./pkg/services -v` to run service layer tests (no fixtures needed).
    *   **Note**: The backend web tests require the `VIKUNJA_SERVICE_ROOTPATH` environment variable to be set to the root of the project. You also need to ensure that the `mage` binary is in your `PATH`.
    *   **Web tests example**: `export VIKUNJA_SERVICE_ROOTPATH=$(pwd) && export PATH=$PATH:$(go env GOPATH)/bin && mage test:web`
    *   **Service tests example**: `export VIKUNJA_SERVICE_ROOTPATH=$(pwd) && go test ./pkg/services -v -run "TestProject_Delete"`
