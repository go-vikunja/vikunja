...
- Never log secrets.
- **No hand-rolled caches.** State that needs a TTL, eviction, or must be shared across replicas goes through `pkg/modules/keyvalue` (`PutWithTTL`, `GetWithValue`, `Del`, `RememberFor`, ...) — never a package-level `map` + `sync.Mutex` with its own expiry. State that is per-process by nature (per-session object caches, parsed static assets) stays in-process.
