...
- Never log secrets.
- **No hand-rolled caches or stores.** Any cached, remembered, rate-limited or otherwise shared runtime state goes through `pkg/modules/keyvalue` (`PutWithTTL`, `GetWithValue`, `Del`, `RememberFor`, ...). Never add a package-level `map` + `sync.Mutex` (or an LRU/TTL implementation) for it: keyvalue already does TTL and eviction, and it is backed by Redis in multi-replica deployments where an in-process map silently desyncs.
