...
- Never log secrets.
- **No hand-rolled caches.** State that needs a TTL, eviction, or must be consistent across replicas (verified credentials, rate limits, remembered lookups) goes through `pkg/modules/keyvalue` (`PutWithTTL`, `GetWithValue`, `Del`, `RememberFor`, ...) — never a package-level `map` + `sync.Mutex` with its own expiry, which silently desyncs between replicas and duplicates what keyvalue already does. Plain in-process state that is per-process by nature (per-session object caches, parsed static assets) stays in-process.
