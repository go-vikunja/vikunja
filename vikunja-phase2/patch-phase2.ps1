...
Write-Host "`n========== Phase 2: Templates + Chains + UI Improvements ==========`n" -ForegroundColor Cyan

if (-not (Test-Path "$ROOT\frontend\src")) {
    Write-Host "[!] Source not found at $ROOT" -ForegroundColor Red; exit 1
}

Write-Host "[1/15] Backend: chain model + task creation..." -ForegroundColor Green
...
Write-Host "[2/15] Backend: step attachment handler..." -ForegroundColor Green
...
Write-Host "[3/15] Migration: step attachments table..." -ForegroundColor Green
Copy-Item "$PATCH\20260224050000.go" "$ROOT\pkg\migration\20260224050000.go" -Force

Write-Host "[4/15] Migration: step time units..." -ForegroundColor Green
Copy-Item "$PATCH\20260224060000.go" "$ROOT\pkg\migration\20260224060000.go" -Force

Write-Host "[5/15] Routes: attachment endpoints..." -ForegroundColor Green
Copy-Item "$PATCH\routes.go" "$ROOT\pkg\routes\routes.go" -Force

Write-Host "[6/15] Gantt: dependency arrows + bar tooltips..." -ForegroundColor Green
...
Write-Host "[7/15] Chain editor + API + create-from-chain..." -ForegroundColor Green
...
Write-Host "[8/15] Drag-to-reorder composable..." -ForegroundColor Green
...
Write-Host "[9/15] Template manager page..." -ForegroundColor Green
Copy-Item "$PATCH\ListTemplates.vue" "$ROOT\frontend\src\views\templates\ListTemplates.vue" -Force

Write-Host "[10/15] i18n + subproject filter fix..." -ForegroundColor Green
...
Write-Host "[11/15] Layout consistency: Labels, Teams, Projects..." -ForegroundColor Green
...
Write-Host "[12/15] Upcoming page: filters + checkbox persistence..." -ForegroundColor Green
Copy-Item "$PATCH\ShowTasks.vue" "$ROOT\frontend\src\views\tasks\ShowTasks.vue" -Force

Write-Host "[13/15] Home page: tasks above last viewed..." -ForegroundColor Green
Copy-Item "$PATCH\Home.vue" "$ROOT\frontend\src\views\Home.vue" -Force

Write-Host "[14/15] Task row: title left, project right..." -ForegroundColor Green
Copy-Item "$PATCH\SingleTaskInProject.vue" "$ROOT\frontend\src\components\tasks\partials\SingleTaskInProject.vue" -Force

Write-Host "[15/15] Building..." -ForegroundColor Green
Set-Location $ROOT
