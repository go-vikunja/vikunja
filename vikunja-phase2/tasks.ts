...
export const useTaskStore = defineStore('task', () => {
	...
	async function update(task: ITask) {
		...
		try {
			// Capture old task before update for cascade comparison
			let oldTask: ITask | null = null
			try {
				oldTask = await taskService.get(new TaskModel({id: task.id}))
			} catch {
	...
	async function checkAndCascadeDates(taskService: TaskService, updatedTask: ITask, oldTask: ITask) {
		try {
