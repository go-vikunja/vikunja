...
func TestTaskServiceCalDAVProject36(t *testing.T) {
	...
	if !has {
		t.Fatalf("Test user15 not found")
	}
	...
	if err != nil {
		t.Fatalf("Failed to get tasks for project 36: %v", err)
	}
	...
	if len(tasks) != 5 {
		t.Errorf("Expected 5 tasks for project 36, got %d", len(tasks))
	}
	...
	for i, task := range tasks {
		...
		if task.ProjectID != 36 {
			t.Errorf("Task %d has wrong project ID: expected 36, got %d", task.ID, task.ProjectID)
		}
	}
