...
test('creates and deletes an account webhook with persisted state', async ({authenticatedPage: page, apiContext, userToken}) => {
	...
	expect((await stored.json()).items).toEqual([expect.objectContaining({target_url: 'https://example.com/account-hook'})])
