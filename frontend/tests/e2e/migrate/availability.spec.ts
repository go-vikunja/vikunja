...
test('runtime configuration gates provider links and operations', async ({authenticatedPage: page, apiContext, userToken}) => {
	...
	const enabled = process.env.VIKUNJA_MIGRATION_TODOIST_ENABLE === 'true'
	...
	for (const provider of [...files, ...oauth]) {
		const available = files.includes(provider) || enabled
		expect(info.available_migrators.includes(provider)).toBe(available)
