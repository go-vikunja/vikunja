...
const sdk = vi.hoisted(() => ({reactionsCreate: vi.fn(), reactionsDelete: vi.fn()}))
...
it('adds only the current user and preserves other reactions', async () => {
	client.setQueryData(taskKeys.detail(1), normalizeTask({id: 1, reactions: {'👍': [{id: 2}], '🎉': [{id: 3}]}}))
	sdk.reactionsCreate.mockResolvedValue({data: {value: '👍', user: {id: 1}}})
