...
import {bucketKeys, kanbanKeys, kanbanQuery, normalizeBucket, type BoardData} from './kanban'
import {createTaskMutationOptions, duplicateTaskMutationOptions, updateTaskMutationOptions, deleteTaskMutationOptions, addTaskAssigneeMutationOptions, bulkCreateTasksMutationOptions, moveTaskMutationOptions, updateTaskPositionMutationOptions, taskWriteBody} from './taskMutations'
import type {Task, PaginatedTask} from '@/client/generated'
const sdk = vi.hoisted(() => ({patchTasksRead: vi.fn(), tasksCreate: vi.fn(), tasksDelete: vi.fn(), tasksDuplicate: vi.fn(), taskAssigneesCreate: vi.fn(), tasksBulkCreate: vi.fn(), taskBucketUpdate: vi.fn(), tasksPositionUpdate: vi.fn(), projectViewBucketsTasksList: vi.fn(), projectTasksList: vi.fn(), bucketsList: vi.fn()}))
...
async function seedScrolledBoard() {
	...
	sdk.projectViewBucketsTasksList.mockResolvedValue({data: {items: [{id: 3, count: 30, tasks: Array.from({length: 30}, (_, index) => ({id: index + 1, project_id: 1}))}]}})
	sdk.projectTasksList.mockResolvedValue({data: {items: [], total: 0}})
	client.setQueryData(projectKeys.detail(1), {id: 1, views: [{id: 2, bucket_configuration_mode: 'manual', default_bucket_id: 3}]})
...
describe('task mutations', () => {
	...
	it('rolls back an optimistic bucket move when the request fails', async () => {
		...
		const original: BoardData = {buckets: [normalizeBucket({id: 3, count: 1, tasks: [{id: 1, bucket_id: 3}]}), normalizeBucket({id: 4, count: 0, tasks: []})], pages: {3: 1, 4: 1}}
	...
	it('rolls back only the caches holding the task', async () => {
		...
		await expect(client.getMutationCache().build(client, updateTaskMutationOptions(true)).execute({id: 1, title: 'new'})).rejects.toThrow('denied')
	...
	it('leaves fetches of caches without the task running', async () => {
		...
		const pending = client.fetchQuery({queryKey: other, queryFn: () => new Promise<PaginatedTask>(resolve => { release = resolve })})
		sdk.patchTasksRead.mockResolvedValue({data: {id: 1, title: 'new'}})
		await client.getMutationCache().build(client, updateTaskMutationOptions(true)).execute({id: 1, title: 'new'})
	...
	it('keeps the cached per-view position when the moved task comes back without one', async () => {
		const client = new QueryClient()
		const key = kanbanKeys.board(1, 2)
		client.setQueryData(key, {buckets: [{id: 3, count: 1, tasks: [{id: 1, position: 42, bucket_id: 3}]}, {id: 4, count: 0, tasks: []}], pages: {3: 1, 4: 1}})
		sdk.taskBucketUpdate.mockResolvedValue({data: {bucket_id: 4, task: {id: 1, position: 0}}})
		await client.getMutationCache().build(client, moveTaskMutationOptions()).execute({project: 1, view: 2, bucket: 4, task: {id: 1}})
		expect(client.getQueryData<BoardData>(key)?.buckets[1].tasks).toMatchObject([{id: 1, position: 42, bucket_id: 4}])
	})
	it('honors the resolved bucket when a recurring task returns to backlog', async () => {
		const client = new QueryClient()
		const key = kanbanKeys.board(1, 2)
		client.setQueryData(key, {buckets: [{id: 3, count: 1, tasks: [{id: 1, done: false, bucket_id: 3}]}, {id: 4, count: 0, tasks: []}], pages: {3: 1, 4: 1}})
	...
	it('renames the moved view bucket on the cached task and leaves other views alone', async () => {
		...
		client.setQueryData(detail, {id: 1, buckets: [{id: 3, project_view_id: 2, title: 'To-Do'}, {id: 8, project_view_id: 5, title: 'Backlog'}]})
		client.setQueryData(kanbanKeys.board(1, 2), {buckets: [{id: 3, project_view_id: 2, title: 'To-Do', count: 1, tasks: [{id: 1}]}, {id: 4, project_view_id: 2, title: 'Done', count: 0, tasks: []}], pages: {3: 1, 4: 1}})
		sdk.taskBucketUpdate.mockResolvedValue({data: {bucket_id: 4, task: {id: 1}}})
		await client.getMutationCache().build(client, moveTaskMutationOptions()).execute({project: 1, view: 2, bucket: 4, task: {id: 1}})
		expect(client.getQueryData<Task>(detail)?.buckets).toMatchObject([{id: 4, project_view_id: 2, title: 'Done'}, {id: 8, project_view_id: 5, title: 'Backlog'}])
	})
	it('renames the moved view bucket from the cached bucket list when no board is cached', async () => {
		...
		const observer = new QueryObserver(client, {queryKey: detail, queryFn})
		const unsubscribe = observer.subscribe(() => {})
		await vi.waitFor(() => expect(queryFn).toHaveBeenCalledTimes(1))
		client.setQueryData(bucketKeys.list(1, 2), [{id: 3, project_view_id: 2, title: 'To-Do'}, {id: 4, project_view_id: 2, title: 'Done'}])
		sdk.taskBucketUpdate.mockResolvedValue({data: {bucket_id: 4, task: {id: 1}}})
		await client.getMutationCache().build(client, moveTaskMutationOptions()).execute({project: 1, view: 2, bucket: 4, task: {id: 1}})
		expect(client.getQueryData<Task>(detail)?.buckets).toMatchObject([{id: 4, project_view_id: 2, title: 'Done'}])
	...
	it('reorders descending lists in their own direction and leaves unsorted ones alone', async () => {
		const client = new QueryClient()
		const descending = taskKeys.list({project: 1, view: 2, params: {sort_by: ['position'], order_by: ['desc']}})
	...
	it('uses the mutation client to move completed tasks to its configured done bucket', async () => {
		...
		const otherBoard: BoardData = {buckets: [normalizeBucket({id: 6, count: 0, tasks: []}), normalizeBucket({id: 7, count: 0, tasks: []})], pages: {6: 1, 7: 1}}
		client.setQueryData(projectKeys.detail(1), {id: 1, views: [{id: 2, done_bucket_id: 4}, {id: 5, done_bucket_id: 7}]})
		client.setQueryData(key, {buckets: [{id: 3, count: 1, tasks: [{id: 1, done: false}]}, {id: 4, count: 0, tasks: []}], pages: {3: 1, 4: 1}})
	...
	it('inserts a created card into a scrolled board without refetching it, while lists still refetch', async () => {
		...
		await client.getMutationCache().build(client, createTaskMutationOptions()).execute({title: 'new', project_id: 1, bucket_id: 3})
	...
	it('inserts a duplicate and a bulk batch into the default bucket of every cached view', async () => {
		const {projectKeys} = await import('./projects')
		const client = new QueryClient()
		const key = kanbanKeys.board(1, 2)
		const otherKey = kanbanKeys.board(1, 5)
		client.setQueryData(projectKeys.detail(1), {id: 1, views: [{id: 2, bucket_configuration_mode: 'manual', default_bucket_id: 4}, {id: 5, bucket_configuration_mode: 'manual'}]})
		client.setQueryData(key, {buckets: [normalizeBucket({id: 3, count: 0, tasks: []}), normalizeBucket({id: 4, count: 0, tasks: []})], pages: {3: 1, 4: 1}})
		client.setQueryData(otherKey, {buckets: [normalizeBucket({id: 6, count: 0, tasks: []})], pages: {6: 1}})
		sdk.tasksDuplicate.mockResolvedValue({data: {duplicated_task: {id: 9, project_id: 1, bucket_id: 0}}})
		await client.getMutationCache().build(client, duplicateTaskMutationOptions()).execute(1)
		expect(client.getQueryData<BoardData>(key)?.buckets.map(bucket => bucket.tasks.map(task => task.id))).toEqual([[], [9]])
		expect(client.getQueryData<BoardData>(otherKey)?.buckets[0].tasks.map(task => task.id)).toEqual([9])
		sdk.tasksBulkCreate.mockImplementation(({body}) => ({data: {tasks: body.tasks.map((task: Task, index: number) => ({...task, id: 20 + index, project_id: 1}))}}))
		await client.getMutationCache().build(client, bulkCreateTasksMutationOptions()).execute([{title: 'a', project_id: 1}, {title: 'b', project_id: 1}])
		expect(client.getQueryData<BoardData>(key)?.buckets[1]).toMatchObject({count: 3, tasks: [{id: 20}, {id: 21}, {id: 9}]})
	})
	it('leaves a filter-configured board to its stale mark', async () => {
		...
		client.setQueryData(projectKeys.detail(1), {id: 1, views: [{id: 2, bucket_configuration_mode: 'filter'}, {id: 5, bucket_configuration_mode: 'manual'}]})
		client.setQueryData(filtered, board)
		client.setQueryData(manual, {buckets: [normalizeBucket({id: 6, count: 0, tasks: []})], pages: {6: 1}})
		sdk.tasksCreate.mockResolvedValue({data: {id: 99, project_id: 1}})
		await client.getMutationCache().build(client, createTaskMutationOptions()).execute({title: 'new', project_id: 1})
	...
	it('renames the cached task bucket when toggling done moves the card', async () => {
		...
		const client = new QueryClient()
		const detail = taskKeys.detail(1, ['buckets'])
		client.setQueryData(projectKeys.detail(1), {id: 1, views: [{id: 2, done_bucket_id: 4, default_bucket_id: 3}]})
		client.setQueryData(kanbanKeys.board(1, 2), {buckets: [{id: 3, project_view_id: 2, title: 'To-Do', count: 1, tasks: [{id: 1, done: false}]}, {id: 4, project_view_id: 2, title: 'Done', count: 0, tasks: []}], pages: {3: 1, 4: 1}})
		client.setQueryData(detail, {id: 1, done: false, buckets: [{id: 3, project_view_id: 2, title: 'To-Do'}]})
		sdk.patchTasksRead.mockResolvedValue({data: {id: 1, project_id: 1, done: true}})
		await client.getMutationCache().build(client, updateTaskMutationOptions()).execute({id: 1, done: true})
		expect(client.getQueryData<Task>(detail)?.buckets).toMatchObject([{id: 4, project_view_id: 2, title: 'Done'}])
		sdk.patchTasksRead.mockResolvedValue({data: {id: 1, project_id: 1, done: false}})
		await client.getMutationCache().build(client, updateTaskMutationOptions()).execute({id: 1, done: false})
		expect(client.getQueryData<Task>(detail)?.buckets).toMatchObject([{id: 3, project_view_id: 2, title: 'To-Do'}])
	})
