...
import {timeEntryKeys, normalizeTimeEntry} from './timeEntries'
import {commentKeys} from './comments'
import {taskKeys, normalizeTask} from './tasks'
...
it('invalidates only the notified task comments and detail', async () => {
	...
	const event = parseServerCacheEvent('notification.created', {name: 'task.comment', notification: {task: {id: 1}}})!
...
it('reconciles timer events without inserting into an unrelated filtered list', async () => {
	const client = new QueryClient()
	const entry = normalizeTimeEntry({id: 4, user_id: 7, start_time: '2026-09-19T08:00:00Z', end_time: null})
