...
it('stores the scheduled deletion date a confirmed token produced', async () => {
	...
	client.setQueryData(accountKeys.user(1), {id: 1, deletion_scheduled_at: '0001-01-01T00:00:00Z'})
	sdk.userDeletionConfirm.mockResolvedValue({})
	const account = {id: 1, deletion_scheduled_at: '2030-01-01T00:00:00Z'}
	sdk.userShow.mockResolvedValue({data: account})
	await client.getMutationCache().build(client, confirmDeletionMutationOptions()).execute('token')
