...
const sdk = vi.hoisted(() => ({webhooksList: vi.fn(), userWebhooksList: vi.fn(), webhooksDelete: vi.fn(), userWebhooksDelete: vi.fn()}))
...
it('uses separate operations and caches for user and project subscriptions', async () => {
	...
	await client.getMutationCache().build(client, deleteWebhookMutationOptions()).execute({scope: {kind: 'project', projectId: 7}, id: 1})
