...
export function useTaskActions(optimisticUpdates = false) {
	...
	async function ensureLabelsExist(titles: string[], context = captureClientRequestContext()): Promise<ResolvedLabels> {
		...
		const found = await Promise.all(wanted.map(async title => {
			...
			try { return {title, label: await createLabel.mutateAsync({title, hex_color: getRandomColorHex()})} } catch {
				assertClientRequestContext(context)
	...
	async function build(input: Partial<Task>, context: ClientRequestContext) {
		...
		const project_id = await findProjectId({project: magicOnly ? null : parsed.project, projectId: input.project_id ?? 0})
		assertClientRequestContext(context)
		...
		return {
			task: buildQuickAddTask(parsed, {...input, project_id}, PREFIXES[mode], matches.filter(user => user !== undefined), defaults),
	...
	async function createNewTasksBulk(entries: {title: string, project_id: number}[]) {
		...
		await runWrites(built.map((item, index) => ({item, index})), async ({item, index}) => {
			...
			try { result.tasks[index] = (await finishTask(task, item, context)).task } catch {
				assertClientRequestContext(context)
			}
		}, config.concurrentWrites)
		return result
	...
	return reactive({
		...
		createNewTask, createNewTasksBulk, findProjectId, ensureLabelsExist,
		...useTaskDragState(),
	})
}
