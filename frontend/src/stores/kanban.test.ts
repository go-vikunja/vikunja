...
const {bucketUpdate, bucketDelete, loadBuckets, refreshProjects, refreshProject, refreshProjectViews} = vi.hoisted(() => ({
	bucketUpdate: vi.fn(),
	bucketDelete: vi.fn(),
	loadBuckets: vi.fn(),
...
vi.mock('@/client/queries/projectViews', () => ({refreshProjectViews}))
...
describe('kanban store: deleteBucket', () => {
	beforeEach(() => {
		...
		refreshProjectViews.mockReset().mockResolvedValue(undefined)
	})
	...
	it.each([
	...
	])('refreshes project views only when deleting a configured bucket: %j', async ({defaultBucket, doneBucket, refresh}) => {
		...
		expect(refreshProjectViews).toHaveBeenCalledTimes(Number(refresh))
		if (refresh) {
			expect(refreshProjectViews).toHaveBeenCalledWith(1)
		}
