...
function getHeight(el: Element) {
	...
	const { display } = htmlEl.style // save display property
	htmlEl.style.display = 'block' // Make it visible
	const height = `${htmlEl.scrollHeight}px` // Get its height
 ...
 */
function forceLayout(el: Element) {
	const htmlEl = el as HTMLElement
...
function removeHeight(el: Element) {
	const htmlEl = el as HTMLElement
	htmlEl.style.height = ''
}
</script>
